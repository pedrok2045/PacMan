 ...
 * sendo ainda esta classe a responsável por "desenhar" o mapa no console.
 */
...
public class Mapa {
	...
	/*
	 * Este método basicamente carrega o mapa a partir de um arquivo txt. Ele lê
	 * o arquivo, pega cada uma de suas linhas e elimina os espaços, em seguida,
	 * pega a quantidade de caracteres naquela linha e atribui este valor à
	 * variável linhas, então pega e começa a preencher o mapa sempre colocando
	 * uma linha e enchendo esta linha com elementos que são gerados de acordo
	 * com o caractere carregado do arquivo de texto, pois para cada um dos
	 * simbolos no arquivo, há uma classe cujo retorno do método print() da
	 * classe Elemento é aquele símbolo. Este método ainda faz a contagem dos
	 * Antagonistas e ElementoAutoMovivel do jogo.
	 */
			...
			try (Scanner scan = new Scanner(in)) {
				...
				while (scan.hasNextLine()) {
					...
					for (int i = 0; i < this.linhas; i++) {
						...
						this.mapa.get(linhaSendoComposta).add(new PosicaoNoMapa(this.gerenciador, this.controlador,
								new Coordenada(linhaSendoComposta, i)));

						// Para cada objeto Posição no Mapa, atribui um
						// elemento, de acordo com o mapa carregado
						ElementoGrafico elemento = this.controlador.getElemento(linha.substring(i, i + 1));
						if (elemento instanceof ElementoMovel) {
							((ElementoMovel) elemento)
									.setPosicaoNoMapa(this.getPosicaoNoMapa(new Coordenada(linhaSendoComposta, i)));
						...
						this.getPosicaoNoMapa(new Coordenada(linhaSendoComposta, i)).inicializa(elemento);
					}
	...
	/*"desenha" o mapa no console chamando o método print() de cada elemento*/
	...
	/*retorna a casa no mapa de acordo com uma posição específica, ou null caso essa posição não seja válida, isto é, esteja fora do mapa.*/
	 ...
	 */
	private boolean ehValido(int posicao, int limiteAEsquerda, int limiteADireita) {
